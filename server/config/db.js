...
const createDatabaseIfNotExists = async () => {
  try {
    await adminConnection.query(`CREATE DATABASE IF NOT EXISTS ${process.env.DB_NAME || 'copay'}`);
    logger.info(`✅ Database ${process.env.DB_NAME || 'copay'} verified`);
  } catch (error) {
...
const connectDB = async () => {
  try {
    ...
    if (error.original?.code === 'ER_BAD_DB_ERROR') {
      ...
      logger.error(`   CREATE DATABASE ${process.env.DB_NAME || 'copay'};`);
      logger.error(`Then grant privileges: GRANT ALL ON ${process.env.DB_NAME || 'copay'}.* TO '${process.env.DB_USER || 'root'}'@'localhost';`);
    }
